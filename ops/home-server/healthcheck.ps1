...
# The ComfyUI restart-handoff logic (Get-PortListenerPid, Test-IsComfyEngine,
# Get-ComfyEnginePids, Restart-Supervised) lives in lib/Restart-ComfySupervised.ps1
# now (conductor/t-185) so restore-shares.ps1 can share the same safe
# stop/reap/verify/start handoff instead of calling `pm2 restart comfyui`
# directly. Dot-source it here, after Write-Log and $comfyDir/$comfyPython are
# already defined above, so the functions below resolve to THIS script's own
# Write-Log and pick up the $comfyDir/$comfyPython this script already computed.
...
# Test-IsComfyEngine lives in lib/Restart-ComfySupervised.ps1 (dot-sourced
# above) alongside Get-PortListenerPid, Get-ComfyEnginePids, and
# Restart-Supervised, since restore-shares.ps1 needs the same "does this
# process look like our ComfyUI engine" check.
...
# Get-ComfyEnginePids and Restart-Supervised (the serialized ComfyUI
# stop/reap/verify/start handoff) live in lib/Restart-ComfySupervised.ps1
# (dot-sourced above), shared with restore-shares.ps1.
