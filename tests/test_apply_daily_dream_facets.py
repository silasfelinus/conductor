...
def write_bundle(path: Path):
    ...
    built = {"records": {"world": {"id": 1}, "vibe": {"id": 2}, "locations": [{"id": 3}], "characters": [{"id": 4}], "rewards": [{"id": 5, "reward_type": "ITEM"}, {"id": 6, "reward_type": "SKILL"}], "scenarios": [{"id": 7}]}}
    path.write_text("---\nproposal: true\n---\n\n" f"<!-- proposal-data\n{json.dumps(proposal)}\n-->\n\n" f"<!-- built-data\n{json.dumps(built)}\n-->\n", encoding="utf-8")


def test_sidecar_applies_world_vibe_and_five_dependent_records(tmp_path, monkeypatch):
    ...
    assert [endpoint for endpoint, _ in calls] == ["/api/dreams/1/facets", "/api/dreams/2/facets", "/api/dreams/3/facets", "/api/characters/4/facets", "/api/rewards/5/facets", "/api/rewards/6/facets", "/api/scenarios/7/facets"]
    built = assign._json_comment(assign.BUILT_RE, path.read_text(encoding="utf-8"))
    assert built["facet_assignments"]["status"] == "complete"
    assert len(built["facet_assignments"]["targets"]) == 7
