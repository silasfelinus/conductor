    ...
    html = module.animation_release_section(
        {
            "animation_release": {
                "state": "fresh",
                "title": "Frost Window Etching",
                "released_label": "Sep 23, 2026 at 5:08 AM Pacific",
                "age_hours": 6,
                "try_url": "https://kindrobots.org/build/animation-manager?effect=frost-window-etching&preview=1",
            }
        }
    )

    assert "New screensaver today" in html
    assert "Frost Window Etching" in html
    assert "Try this screensaver" in html
    assert "effect=frost-window-etching&preview=1" in html
