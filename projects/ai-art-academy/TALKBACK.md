...
## 2026-07-16 10:35 UTC | Worker → Reviewer/Silas | ai-art-academy/t-004 | pattern

type: pattern

**Subject:** KR_API_TOKEN is now provisioned and is no longer a blocker — stop
re-confirming its absence every cycle.

**Detail:**
- Silas added KR_API_TOKEN to the Claude Code environment's env-var field. Verified
  live in this session: `env | grep KR_API_TOKEN` returns a real value, and the art
  scripts read it via `os.environ.get("KR_API_TOKEN")` — so agent/worker sessions
  spawned from this environment now have it. Prior cycles' "absent across N sessions"
  reports (t-004, t-009, and several t-010 run-log entries) were all from cycles that
  ran BEFORE it was provisioned; they are stale, not wrong-at-the-time.
- The token has TWO independent homes, which is why it felt like "submitting to
  multiple locations": (a) the environment env-var field → for Claude sessions/workers
  (now set); (b) the GitHub Actions repo secret `secrets.KR_API_TOKEN` → for the 7 cron
  workflows (auto-art-generate.yml, hourly-conductor.yml, worker.yml, etc.). The env
  field does NOT reach CI, so if art is generated by the scheduled workflow, the repo
  secret must be set too. kind_robots itself never reads KR_API_TOKEN — it validates the
  bearer token from the request header — so putting it in kind_robots `.env`/nuxt
  config/Vercel under that name does nothing.
- Rendering (incl. Kontext image-to-image) uses a SEPARATE secret: `KR_RELAY_TOKEN`
  (+ `KR_RELAY_USER_ID`) on the home GPU box (ops/home-server/relay_agent.py), not
  KR_API_TOKEN. And the kind_robots production DB outage (kind-robots/t-022) still
  blocks enqueue/validation independently of the token.

**Suggested action:** future ai-art-academy cycles should NOT re-report a
KR_API_TOKEN-absence blocker on t-004/t-009. Instead, a token-bearing session should
verify the live kind_robots API is reachable (DB up) — e.g. `fetch_todos.py` or a GET —
and if so flip t-004/t-009 to `ready` and run. Corrected the t-004/t-009/t-010 notes to
say this (rides in conductor PR #616). Museum-egress 403 (t-008/t-013) is unaffected and
remains a genuine, separate block.
