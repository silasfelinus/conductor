...
## 3. Per-style teaching notes (all 30 movements)
...
| 32 | Hudson River School (`hudson-river-school`) | prompt | Medium | The land itself as a moral and spiritual stage — sublime, abundant, and (deliberately) apparently unoccupied | Likely fails toward a generic "dramatic sky filter" over the photo's existing composition rather than the movement's real structure; lean on the remix_hint's explicit "tiny figures for scale" and "shadowed foreground opening toward luminous golden light" to check those staged compositional roles are actually present, and discuss the Manifest Destiny/Indigenous-erasure framing honestly rather than teaching the beauty alone |
| 33 | Precisionism (`precisionism`) | prompt | Medium | Ordinary American factories and grain elevators rendered as pristine, monumental geometry — what disappears when industry becomes design | Model may under-cook toward a lightly-processed photo, or over-cook into a glossy science-fiction city with futuristic materials and neon lighting the movement never had; lean on the remix_hint's explicit "crisp geometric masses," "controlled daylight and exact-edged shadows," and "remove visual clutter" to keep it early-20th-century industrial rather than either extreme |
| 34 | Pre-Raphaelite Brotherhood (`pre-raphaelite`) | prompt | Medium | A rejection movement built from a return-to-the-past idea — jewel-bright color and obsessive natural detail as a deliberate revolt against Royal Academy convention | Likely under-cooks toward a generic "old oil painting" filter with warm varnish tone rather than the movement's actual saturated jewel color and hyper-detailed hair/fabric/foliage; lean on the remix_hint's explicit "saturated jewel-toned color" to keep it vivid rather than sepia-toned, and discuss how symbolic objects (not incidental still-life) drive the composition |
| 35 | Arts and Crafts Movement (`arts-and-crafts`) | prompt | Medium | Design ethics, not one look — visible handwork, honest materials, and integrated interiors as a reply to industrial mass production | The model must invent a plausible botanical/animal repeat where the source photo has none while keeping visible "hand-process" texture; likely fails toward a generic flat decorative-pattern filter that skips block-print/embroidery/joinery texture — lean on the remix_hint's explicit "visible craft structure" and "natural materials," and discuss the movement's affordability contradiction (dignified-labor ideals vs. expensive handmade objects) honestly |
| 36 | Italian Futurism (`italian-futurism`) | prompt | **Hard** | Depict speed and simultaneity itself — a moving subject shown as several instants compressed into one image | Likely collapses into either a static faceted image (losing the motion) or a generic photographic motion-blur filter (losing the constructed force-line/facet structure); lean on the remix_hint's explicit "overlapping, semi-transparent repetitions" and "sharp diagonal force lines" to check for constructed dynamism rather than a blur effect, and discuss the movement's later alignment with Fascism and the manifesto's glorification of violence honestly rather than teaching speed/machinery worship as neutral |
| 38 | Fauvism (`fauvism`) | prompt | Medium | Color freed entirely from observed reality — the same subject, but every local color choice is expressive rather than descriptive | Likely under-cooks toward a generic "brightened/saturated photo" that keeps naturalistic local color instead of breaking it (a green face, an orange tree trunk); lean on the remix_hint's explicit instruction to abandon local color accuracy entirely and keep the bold dark contour outlines Fauvist paintings use to hold flat color areas together, rather than just raising saturation on the source photo |
