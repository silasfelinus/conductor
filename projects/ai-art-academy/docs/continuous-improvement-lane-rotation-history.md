# AI Art Academy — continuous-improvement lane-rotation history

Archived historical "Rotation state" prose from
`continuous-improvement-checklist.md`, moved out 2026-07-27
(ai-art-academy/t-010, lane 2 roadmap-accuracy cycle) for the same reason
t-039 (2026-07-26) moved the roadmap task's own `RAN ...` paragraphs into
`continuous-improvement-run-log.md`: the checklist's per-cycle rotation
narrative had grown to ~1,290 lines going back to 2026-07-19, making the
checklist slow to read for the one thing a session actually needs from
it (which lane runs next) — and the roadmap task's own structured
`continuous_improvement:` field (`last_lane`/`next_lane`/`last_run`/
`last_pr`, added by t-039) already supersedes this prose as the live-state
pointer. Read that field for current state; read this file only for
historical provenance (what ran, what was found, how it was verified) on
a specific past cycle.

Entries are unedited except for de-indentation — nothing was reworded or
summarized in the move (verified: this file's entry content was diffed
byte-for-byte against the removed checklist section before commit).
Most-recent-first, oldest at the bottom, matching the checklist's own
prior ordering.
