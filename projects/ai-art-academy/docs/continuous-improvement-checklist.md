...
| Style previews | UPDATE (2026-07-28 lane 4): all 36 `kind-robots-academy-style-preview-*` requests are `status: done` and all 36 are now confirmed live at `https://media.acrocatranch.com/images/academy/styles/<slug>.webp` (spot-checked every slug added or touched since the last confirmed-live check — pre-raphaelite, arts-and-crafts, italian-futurism, the-nabis, hudson-river-school, precisionism, greek-vase-painting — all HTTP 200) and wired into `academyStyles.ts`'s `previewImageSrc` field | No gap — generation, delivery, and front-end wiring are all caught up with curriculum coverage. Re-queue a preview for any future new movement (lane 4) promptly so this gap doesn't reopen |
