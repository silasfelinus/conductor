...
| Style previews | UPDATE (2026-07-28 lane 3): 37 `kind-robots-academy-style-preview-*` requests now exist (36 confirmed live at `https://media.acrocatranch.com/images/academy/styles/<slug>.webp` as of the last spot-check, plus `fauvism` — generated this cycle, ArtJob 2775/ArtImage 13125, retained in `projects/process/fauvism.webp` pending relay/media-path delivery, not yet confirmed live) | Re-verify `fauvism.webp` resolves HTTP 200 at the production media host once relay delivery completes, same as every prior retained-not-yet-delivered entry. Re-queue a preview for any future new movement (lane 4) promptly so this gap doesn't reopen |
