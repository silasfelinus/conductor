...
The Academy currently has 34 movement entries in `curriculum-outline.md` —
Pre-Raphaelite Brotherhood (§34) was added this cycle (2026-07-27, lane 4):
...
|---|---|---|
| Lesson seed entries | 33 of 34 movements in curriculum-outline.md are synced to `academyStyles.ts` — The Nabis, Hudson River School, and Precisionism landed 2026-07-27T10:20:00Z via kind_robots PR #1045. Pre-Raphaelite Brotherhood (§34, added this cycle) is not yet synced | Sync `pre-raphaelite` into `academyStyles.ts` the same way (`exampleWorks`/`previewImageSrc` still deferred on all of them, see the row below) |
...
| Teaching scaffold | Written in `docs/teaching-notes.md`, covering 33 of 34 movements through Precisionism (row 33); Pre-Raphaelite Brotherhood (§34, added this cycle) has no row yet; wired into `academy-style-detail.vue`'s Try It / Reflect sections (t-023, done — verified 2026-07-18 via `grep -n "Try it\|Reflect" components/academy/academy-style-detail.vue` on kind_robots main) | Add a Pre-Raphaelite Brotherhood row (34) to `teaching-notes.md`'s per-style table |
