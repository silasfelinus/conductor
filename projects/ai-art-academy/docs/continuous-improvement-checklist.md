...
| Style previews | UPDATE (2026-07-29 lane 3): the 2026-07-28 `fauvism` generation (ArtJob 2775/ArtImage 13125) never actually landed — it was generated without a matching `art-prompts.yaml` request entry, so `distribute_images.py` couldn't route it and it sat in `projects/process/unmatched/fauvism.webp` indefinitely (see t-010's 2026-07-29T04:20Z stranded-state note). Re-queued properly this cycle: added the `kind-robots-academy-style-preview-fauvism` request entry, regenerated (job 2825, ArtImage 13267), staged at `projects/process/fauvism.webp` (256x256), confirmed via `distribute_images.py --dry-run` it now matches and is correctly slated for `public/images/academy/styles/fauvism.webp`. Deleted the old unmatched stray. 36 other movements remain confirmed live at `https://media.acrocatranch.com/images/academy/styles/<slug>.webp`; fauvism itself still 404s pending relay delivery | Re-verify `fauvism.webp` resolves HTTP 200 at the production media host once relay delivery completes, same as every prior retained-not-yet-delivered entry. Re-queue a preview for any future new movement (lane 4) promptly, and always create the `art-prompts.yaml` request entry *before* generating — this cycle's whole fix was closing that ordering gap |
