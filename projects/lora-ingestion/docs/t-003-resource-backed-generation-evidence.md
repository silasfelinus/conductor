...
## 2026-09-15 update — second reproduction of the exact same hang, on the same checkpoint

conductor/t-147 was closed as mitigated (Krea/Flux LoRA compatibility split merged, kind_robots PR #2416 — a genuine fix for a *different* bug: stale Flux LoRAs being offered on the Krea2 engine). It did not touch anything on the relay/model-loading side, so it could not have fixed the Flux.2 Klein hang documented above — and it didn't.

**Session 2026-09-15T10:30Z** (recorded in the task note, not yet in this doc): found a genuinely new Flux.2-generation LoRA Resource that didn't exist in the 2026-08-10/09-05 searches — Resource 1123 (`Flux_2-Turbo-LoRA_comfyui`, `localPath Flux/SFW/Flux_2-Turbo-LoRA_comfyui.safetensors`, `generation "Flux.2 D"`). Queued ArtJob **22837** via `POST /api/art/enqueue` using this Resource. Confirmed via `GET /api/art/queue/22837` that the workflow wired `LoraLoaderModelOnly.lora_name` to this Resource's exact `localPath` onto the `flux2_dev_fp8mixed.safetensors` checkpoint — Resource-path resolution correct again. The job was still `PENDING` (behind another job) when that session ended, so it re-armed `t-003` to `ready` rather than escalating, explicitly leaving "poll 22837 to a terminal state" as the next step.

**This session (2026-09-15T11:5x UTC)**: polled `GET /api/art/queue/22837` — status `RUNNING`, `claimedAt: 2026-09-15T10:43:51.968Z`, `claimedBy: Silas-PC`. `GET /api/art/queue/stats` at 11:51:13Z (67+ minutes after claim) showed `staleRunningCount: 1` naming this exact job, `queueDepth.PENDING: 3` (three newer jobs queued and waiting behind it), and `oldestPending` was a *different*, newer job (22838) — i.e. the relay's single concurrent slot was fully wedged on 22837 again, blocking the whole production queue a second time, on the same checkpoint (`flux2_dev_fp8mixed.safetensors`) as the 2026-09-05 hang. `scripts/check_render_box.py` independently confirmed: `render box DOWN ... the pipeline is not rendering — 1 stale RUNNING claim(s) with 3 PENDING job(s) still waiting and nothing else moving`.

Cancelled ArtJob 22837 (`POST /api/art/queue/22837/cancel`, same recovery action as 2026-09-05's 21616) — this is this task's own job (`projectSlug: lora-ingestion-t003`), so cancelling it matches the precedent set for un-wedging a task-owned stuck claim. `staleRunningCount` dropped to 0 immediately, and — notably different from 2026-09-05 — the relay picked up new work right away: `check_render_box.py` reported `UP ... Engine heartbeat: Comfy: heartbeat healthy 0.9 minutes ago` within a minute of the cancel. The relay process itself was not dead; it was specifically wedged loading/running this Flux.2 Klein checkpoint pairing, twice in a row, with no other failures in between.

**Updated conclusion**: this is now a **reproduced, checkpoint-specific hang** (2 for 2 attempts using `flux2_dev_fp8mixed.safetensors`, on two different LoRA Resources, five weeks apart), not a one-off. Both attempts used the application's correct, Resource-resolved workflow — the bug is not in Kind Robots' Resource/path plumbing, it is in what happens when the relay actually tries to run this specific checkpoint (first-time large model load, an incompatible dtype/precision path, or a ComfyUI node erroring in a way that hangs rather than raises — `error: null` both times rules out a clean rejection). This needs hands-on diagnosis on the Silas-PC relay itself (watch the ComfyUI console/log while a `flux2_dev_fp8mixed.safetensors` job runs) — nothing reachable from this sandbox can do that. Filed `conductor/t-165` for tracking (a fresh task rather than reopening the closed, unrelated-cause t-147). `t-003` returns to `needs-human` (hard — the underlying diagnosis needs physical/console access to hardware this sandbox cannot reach) pending that.

Krea2 remains untestable for the same reason as before (zero compatible Resources in the catalog) — unchanged, not agent-actionable.
