...
## 2026-09-17 | Agent (Claude, scheduled conductor run, worker) | appmaker/t-010 | resolution

**Decision:** merged silasfelinus/kind_robots#2802 (squash `8cff47a`), all required CI green
(51/51, only the non-required "Build production image" deploy job still in flight at merge
time, per established precedent). Closed `t-010` to `done`; filed `t-015` for the remaining
executor scope.

**What happened:**
- `t-010` had sat since 2026-08-14 with three real open design questions blocking any
  implementation (history-preservation mechanism, ownership model, target-repo
  prerequisite) — but Silas answered all three in a 2026-09-07 default-recommendation
  decision already recorded in the task's own note (squash graduation over full
  git-subtree-split replay, admin-only since a monorepo app has no individual owner,
  require an already-connected/already-granted repo). Reading that note fully turned an
  apparently-stuck `gate_human: true` task into straightforwardly buildable work.
- Split the landable half from the genuinely irreversible half rather than building both
  in one pass: `POST /api/appmaker/graduate-request` (admin-only, validates the app is a
  real `apps/<slug>/` folder, refuses if an `AppRepo` already exists for the slug, checks
  installation ownership/suspension, and checks `listInstallationRepositories()` before
  trusting client-supplied owner/repo) files a Todo and an `AppRepo` row but pushes
  nothing to GitHub — same shape as the two existing self-serve flows
  (`scaffold-request.post.ts`, `github/create-app.post.ts`), reused directly rather than
  reinvented.
- Read `appmakerGithub.ts`'s existing `pushScaffoldBranchAndOpenPr` before assuming it
  could be reused for the executor: its per-file Contents-API PUT loop produces one commit
  per file, the opposite of "squash" — so the filed Todo explicitly says no executor
  exists yet rather than naming a `Call: POST /api/...` endpoint that doesn't exist (the
  pattern `create-app.post.ts`'s own Todo uses), and the real executor was split off as
  `t-015` (`gate_human: true`, since it mints a real installation token and writes to a
  real external repo plus a monorepo-removal PR) rather than either building it unreviewed
  in the same PR or leaving the remaining scope implicit in `t-010`'s note.
- Added `verifyAppmakerGraduateRequestGuard.ts` + self-test following this project's
  established narrow-textual-checker convention (`verifyAppmakerScaffoldCollisionGuard.ts`,
  `verifyAppmakerPendingScaffoldPatternGuard.ts`) — pins the admin-gate, collision-check,
  installation-ownership-scope, suspended-check, grant-validation, and Todo-title↔regex-sync
  invariants; self-test proves each of 7 dropped-invariant fixtures fails its own specific
  assertion rather than trusting the checks by inspection.
- Verified: `eslint`/`vue-tsc --noEmit`/`prettier --check` clean on every changed/new file;
  new guard's self-test and main check both pass; all 4 pre-existing AppMaker guards
  (`pending-scaffold-pattern`, `scaffold-collision`, `slug-candidate`, `fleet-description`)
  re-run clean, no regressions; `git status --porcelain` scoped to exactly 7 files.

**What was good:** treating "the design is now decided" and "the design is now safe to
build in full" as separable judgments — the note's decision unblocked real work, but
didn't make the executor's real-repo-writing risk go away, so only the reversible half
shipped this cycle.

**Kaizen task:** `t-015` (filed directly from this close-out, not a generic suggestion) —
build the squash-graduation executor via the Git Data API. Its note carries one open
question forward: whether GitHub's Git Data API can create the very first commit/ref in a
genuinely empty target repo, or whether the repo needs an initial commit via the regular
Contents API first.

---
_Generated by [Claude Code](https://claude.ai/code/session_013irER5zfZ6CaBmMkMpkGQ8)_
