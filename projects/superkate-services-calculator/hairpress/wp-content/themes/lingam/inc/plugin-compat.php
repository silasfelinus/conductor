...
add_action( 'after_setup_theme', 'lingam_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function lingam_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'lingam_construct_sidebars' );
}

if ( ! function_exists( 'lingam_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'lingam_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function lingam_woocommerce_start() { ?>
...
if ( ! function_exists( 'lingam_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'lingam_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function lingam_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'lingam_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lingam_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function lingam_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'lingam_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'lingam_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lingam_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function lingam_bbpress_css() {
...
if ( ! function_exists( 'lingam_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lingam_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function lingam_buddypress_css() {
...
if ( ! function_exists( 'lingam_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lingam_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function lingam_beaver_builder_css() {
