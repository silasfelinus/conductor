( function( api ) {
	...
	api.controlConstructor['lingam-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.lingam-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the lingam_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'lingam_get_all_google_fonts_ajax',
								lingam_customize_nonce: lingam_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.lingam-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.lingam-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.lingam-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.lingam-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.lingam-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.lingam-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.lingam-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.lingam-font-variant' );
			...
			control.container.on( 'change', '.lingam-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.lingam-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.lingam-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.lingam-font-family select' ).selectWoo();
	jQuery( '.lingam-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".lingam-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.lingam-font-variant' ).hide();
		}
	});
