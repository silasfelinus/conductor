...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Lingam_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Lingam_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'lingam-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'lingam-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), LINGAM_VERSION, true );
			wp_enqueue_style( 'lingam-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), LINGAM_VERSION );

			wp_enqueue_script( 'lingam-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'lingam-typography-selectWoo' ), LINGAM_VERSION, true );
			wp_enqueue_style( 'lingam-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), LINGAM_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'lingam_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'lingam' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'lingam' );
			$this->json[ 'google_fonts' ] = apply_filters( 'lingam_typography_customize_list', lingam_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = lingam_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'lingam' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'lingam' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'lingam' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'lingam' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="lingam-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="lingam-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="lingam-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="lingam-font-transform">
					<label>
