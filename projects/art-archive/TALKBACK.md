...
## 2026-09-19 | Agent (Claude, scheduled conductor run) | art-archive/t-030 | resolution

type: resolution

**Subject:** Merged kind_robots#2876 -- fixed a real cache-correctness bug found while measuring t-030's own gating question, closed the roadmap task to `done` on that finding rather than building the originally-titled persisted-checkpoint feature.

**Detail:**
- t-030's note explicitly gated any implementation behind a timing measurement ("confirm with a rough timing measurement against a real large archive before adding persisted state... only build real checkpoint/cursor tracking if that measurement shows the walk itself, not the hashing, is the actual bottleneck"). Real `PRIVATE_PATH` access is confirmed unavailable in every sandbox (art-archive/t-019's own note), so built a synthetic local benchmark instead (20k and 60k placeholder files, comparing a cold full-hash scan against a warm scan using a fully-populated `knownFiles` cache map).
- The warm pass reported **0 cache hits** on the first run -- not the expected outcome, and not a benchmark artifact. Traced it to `scanArchiveRoot()`'s cache-hit check comparing a live `fs.stat().mtimeMs` (confirmed empirically to be a non-integer float with sub-millisecond precision on this sandbox's filesystem) against `known.fileMtimeMs`, which is always an integer because it round-trips through a JS `Date` both in `ScannedArchiveFile.fileMtime` and in `loadKnownArchiveFiles()`'s `.getTime()` read from the DB. The strict equality check between a float and its own truncated integer almost never held, meaning t-018's entire incremental-hash cache -- shipped and closed as `done` -- had likely never actually engaged in production.
- Also found that the *existing* test claiming to cover this ("an unchanged known file is served from cache") had a fixture bug of its own: it built `known.fileMtimeMs` directly from an untruncated live `fs.stat()` call rather than through the real Date-round-trip path, so it could never have caught this regression -- an unrealistic mock that happened to still pass for the wrong reason.
- Fixed the comparison (truncate the live side to whole milliseconds before comparing -- no DB/schema change needed, `ArchiveEntry.fileMtime` is already `DATETIME(3)`), fixed the existing test's fixture to match real usage, and added a dedicated regression test. Re-ran the benchmark: 100% cache hits, ~2.4x faster warm-vs-cold even with tiny 2KB placeholder files (real archive images are much larger, so hashing cost -- which scales with file size, unlike walk/stat cost -- should widen this gap substantially in production).
- This answers t-030's own question directly: with the cache actually working, a restarted scan's re-walk of already-known files is cheap, so the walk itself is not the bottleneck at scale. Closed the task `done` on this finding rather than building checkpoint/cursor tracking that the task's own note says isn't warranted once that's true.

**What was good:**
- N/A (this session was both Worker and Reviewer for this task).

**What to improve:**
- Nothing to flag against a prior cycle -- this bug predates any session in this project's visible history (shipped with t-018). Standing note: an "obviously correct" cache-comparison test is worth checking that its fixture actually exercises the real persistence path (DB round-trip, Date objects, etc.), not just a conveniently-constructed in-memory value -- this is the second time in this project a plausible-looking test missed the real production path (see t-027's TALKBACK entry re: testing for a COMFY workflow-construction path that doesn't actually exist).

**Kaizen task:** t-036 -- surface the scanner's cache-hit rate in `reconcileArtArchive.ts --dry-run` output, so a future regression in cache effectiveness is visible rather than silent.
