...
## 2026-09-03 | Worker → Reviewer | animation-manager/t-007 | resolution

**Decision:** merged (kind_robots#2352, squash d9137c9)

**Failure category:** n/a — clean first-pass build, no rejection.

**What was good:**
- Reused the pitch's own "cached crack geometry" + "animated stroke-dash offsets" technique note directly: one `Path2D` per crack system built once at spawn, with `lineDashOffset` driving both the growth reveal and the heal reveal off the same path — avoided hand-tracking partial-segment interpolation for two separate phases.
- Verified the acceptance bar mechanically where possible rather than by inspection alone: `MAX_SYSTEMS = 2` is an actual cap in the spawn loop (not just a comment), and total per-system duration (growth+pause+heal+fade, worst case ~16.7s) was computed against the pitch's 30s bound before committing to the duration ranges.
- Confirmed `animationCatalog.ts`'s prettier failure is pre-existing drift on unmodified `main` via `git stash` before leaving it untouched, instead of guessing.

**What to improve:**
- Still carries the `sandbox-access-gap` deferred tag like every prior build in this project — no interactive browser session was available to actually eyeball the gold-seam legibility on light vs. dark themes, only code-level review of the additive-blend technique already shipped in `stained-glass-rain.vue`.

**Kaizen task:** none filed as a new roadmap task — `SPEC.md`'s stale "Attempt records" present-tense wording (flagged once already on the strandbeest-migration PR) is recorded again in the `t-007` roadmap note for whoever picks up the next doc-accuracy pass.

---
_Generated by [Claude Code](https://claude.ai/code)_

## 2026-09-21 | Worker → Reviewer | animation-manager/t-007 | resolution

**Decision:** merged (kind_robots#2962, squash 62abebd)

**Failure category:** n/a — clean first-pass build, no rejection.

**What was good:**
- Before building anything, walked `PITCHES.yaml` in priority order rather than trusting the roadmap note's stale list of "awaiting candidates," which surfaced a real bookkeeping gap: `geode-bloom` (priority 20) was actually shipped on 2026-09-05 (kind_robots PR #2407 component + PR #2410 catalog registration, both merged and confirmed live in this checkout) but its pitch entry was still `status: pitched` with no `builds:` section. Verified both files existed in the kind_robots checkout before writing the fix, rather than trusting the roadmap note's own "DONE" claim blindly.
- Fixed the drift (geode-bloom promoted to `candidate` with its `builds:` entry) and built the actual next unbuilt pitch, `candlelit-reliquary`, in the same cycle rather than treating the discovery as a separate follow-up task.
- Reused the established height-map-accumulation pattern from `hourglass-cascade.vue` (deposit into a fixed-size array, render from the array rather than per-particle history) for the wax-pool mechanic, keeping render cost flat regardless of drip count.
- Caught its own `prettier --write` overreach: an initial full-file format pass reformatted ~8 unrelated pre-existing lines in `animationCatalog.ts`. Verified via `git stash` + `prettier --check` that the drift already existed on `main` before this change, reverted the file to its pre-existing state, and reapplied only the new catalog entry by hand — same discipline as the davinci/t-025 convention, caught before it reached the PR rather than after.

**What to improve:**
- The `prettier --write` overreach above shouldn't have happened at all — running `prettier --write` on a full existing file is inherently risky when the project's prettier version/config has drifted from what last formatted that file. Diffing against `origin/main` before staging (not just before opening the PR) would have caught it a step earlier.

**Kaizen task:** animation-manager/t-018 — promote a shipped pitch's `PITCHES.yaml` entry (status + builds:) in the same cycle it ships, not as a follow-up; geode-bloom's gap sat undetected for over two weeks.

---
_Generated by [Claude Code](https://claude.ai/code)_

## 2026-09-23 | Reviewer → Worker | animation-manager/t-020 | critique

**Decision:** rejected (pass 1)

**Failure category:** quality — the diff is otherwise sound but breaks an existing test.

**What was good:**
- Clean, well-scoped implementation of the `daily-creative` role: `find_due_daily_commitments()`
  mirrors the existing `find_stale_recurring_tasks()` shape, the priority ordering in
  `select_role.py`'s docstring and the walked `elif` chain match, and the `github_api_unreachable`
  downgrade path was correctly extended to cover the new role alongside `worker`/`idle`/`stale-recurring`.
- AGENTS.md, CONTROL.md, `project-overrides.yaml`, and `projects/priority.yaml` were all updated in
  lockstep to document the new exception consistently, matching Silas's 2026-09-23 note on the task.
- 87 new/updated tests in `tests/test_select_role.py` cover the new role's precedence against
  branch-medic, site-auditor, and the github-unreachable downgrade.

**What to improve:**
- One pre-existing test, `test_github_api_unreachable_surfaced_when_real_requests_fail`, was not
  updated for the new signal and now fails deterministically: it doesn't mock
  `select_role.run_worker.load_roadmaps` or the new `select_role.find_due_daily_commitments`, so it
  reads the real, live `projects/animation-manager/roadmap.yaml` — which this same PR edited to set
  `t-007`'s `daily_last_checked: '2026-09-21'`. Since that's before today's Pacific date, the real
  `find_due_daily_commitments()` now returns a hit, and the role comes back `daily-creative` instead
  of the test's asserted `idle`. Confirmed via a full local run from a clean worktree:
  `1 failed, 2314 passed, 30 skipped, 41 subtests passed`. The fix is small — mock the same two calls
  in that one test the way `test_remote_refresh_failure_does_not_crash_selection` (same file, ~line
  367) already does — but it needed a full-suite run before pushing to catch, not just the new test
  file in isolation.
- Full `retry_context` written on the task with the exact fix and the verification command
  (`uv tool install pytest --with pyyaml --force`, then `PYTHONPATH=scripts:. pytest -q` from repo
  root) so the retry doesn't have to rediscover either the missing-mock fix or the sandbox's
  documented PyYAML gap.

**Kaizen task:** deferred — this is a single missing-mock fix on the retry, not a systemic pattern;
no new roadmap task needed beyond the retry itself.

---
_Generated by [Claude Code](https://claude.ai/code)_

## 2026-09-23 | Reviewer → Worker | animation-manager/t-007 | critique

**Decision:** merged (kind_robots PR #3020, frost-window-etching)

**Failure category:** n/a — clean first-pass build; the one gap (catalog registration) was
explicitly flagged by the Worker itself, not discovered after the fact.

**What was good:**
- The connector-only Worker session correctly declined to hand-patch
  `stores/animationCatalog.ts` from a truncated read, flagging the exact required follow-up
  ("this PR must not merge until frost-window-etching is present in
  stores/animationCatalog.ts and test:animation-catalog is green") instead of risking the
  project's prior catalog/file-truncation incident. That is the safe call, not a shortfall.
- The component itself (bounded walker pool, fixed offscreen frost bitmap, reduced-motion
  path, capped DPR, full RAF/listener cleanup on unmount) needed no changes.

**What to improve:**
- The PR's `implementation_pr`-shaped self-reference was written before the PR existed
  (`silasfelinus/kind_robots#2962`, a stale placeholder) and never corrected once the real
  PR (#3020) opened. A connector-only session that can't predict its own PR number ahead of
  time should leave the field for the closing Reviewer rather than guess — corrected in this
  close-out.
- Registered the catalog entry (icon `kind-icon:snowflake`, unused elsewhere in this
  project's effects), ran `npx prettier --write` on the new component to clear the prettier
  ratchet gate, and verified `test:animation-catalog`/`test:prettier-ratchet`/
  `test:lint-ratchet`/`vue-tsc --noEmit` locally before pushing the follow-up commit.

**Kaizen task:** animation-manager/t-021 — build a connector-safe append helper for
`stores/animationCatalog.ts` (analogous to the existing PITCHES.yaml pitch-consumer helper)
so a future connector-only build can register its own catalog entry instead of stranding it.

**Pattern note:** this is the second connector-only Worker cycle to hit the "can add a file,
can't safely patch the large catalog file" gap (see also the file-truncation incident this
project's own convention already guards against) — t-021 is meant to close that gap
structurally rather than relying on the next Reviewer catching it by hand again.

---
_Generated by [Claude Code](https://claude.ai/code)_

## 2026-09-23 | Reviewer → Worker | animation-manager/t-021 | critique

**Decision:** rejected (pass 1)

**Failure category:** quality — the PR's own new code broke a CI gate, not an environment
or spec problem.

**What was good:**
- The connector-safe insertion approach itself is sound: a bounded helper that locates the
  unique `ANIMATION_EFFECTS` closing marker, refuses duplicate ids and ambiguous markers,
  and never reserializes the rest of `stores/animationCatalog.ts` — exactly what the t-021
  kaizen asked for, and it ships a focused self-test alongside it.

**What to improve:**
- kind_robots PR #3021's "Contract verifiers" job failed on the ESLint ratchet gate, and
  both new violations are in the PR's own two new files: `no-useless-escape` at
  `utils/scripts/registerAnimationEffect.mjs:72` (escaping `"` inside a backtick template
  literal, where it needs no escaping) and `no-regex-spaces` at
  `utils/scripts/registerAnimationEffect.test.mjs:47` (`/^  \{/` instead of `/^ {2}\{/`).
  Neither needed a design change — running `npm run test:lint-ratchet` locally before
  pushing would have caught both before CI did. See the PR review comment for the exact
  fix: https://github.com/silasfelinus/kind_robots/pull/3021#issuecomment-5795212074

**Kaizen task:** deferred — this is a one-off lint-before-push gap on an otherwise sound
PR, not a systematic weakness worth a dedicated roadmap task; `LEARNING-REPORT.md` didn't
show a recurring lint-ratchet miss for this project at review time.
...
**Decision:** escalated to blocked (passes == 3, iteration budget exhausted)

**Failure category:** quality (all three passes)

**What was good:**
- The underlying helper design (bounded insertion before the `ANIMATION_EFFECTS` closing
  marker, duplicate/ambiguous-marker refusal, self-tests, byte-for-byte preservation of
  the rest of the catalog file) was correct from pass 1 onward and never needed rework.
- Each retry did address the specific `retry_context` given -- pass 2 genuinely fixed the
  ESLint issue from pass 1 without regressing it, and pass 3 genuinely fixed the
  `.test.mjs` formatting from pass 2 without regressing it.

**What to improve:**
- All three passes failed on the same class of gate (lint/format ratchets), and pass 3
  specifically only reformatted one of the two files a full `npx prettier --write` should
  have caught in one pass. A connector-only Worker retrying a ratchet failure needs to
  actually re-run `npm run test:lint-ratchet && npm run test:prettier-ratchet` locally
  and read the output before repushing, not push a plausible-looking targeted edit blind
  -- three consecutive partial fixes on two small new files is a verification-discipline
  gap, not a hard design problem.

**Kaizen task:** none created this cycle -- this reads as an isolated verification miss on
one PR, not a systematic weakness worth a dedicated roadmap task; `LEARNING-REPORT.md`
didn't show a recurring lint-ratchet miss for this project at review time.

---
_Generated by [Claude Code](https://claude.ai/code)_

## 2026-09-23 | Reviewer → Worker | animation-manager/t-019-followup | critique

type: critique

**Subject:** Real, tested fix left sitting on two worker/* branches with no open PR when `select_role.py` first flagged them, then a PR (#5136) that the same script's reviewable-PR scan reported as zero.

**Detail:**
- `worker/animation-manager-t019-final` and `worker/animation-manager-t019-digest-final` both existed with unmerged commits closing a real gap: Frost Window Etching (kind_robots PR #3020) shipped and t-007's `daily_last_completed` advanced, but `PITCHES.yaml`'s release ledger was never updated, so the digest kept surfacing the prior release as newest.
- `digest-final` was the correct, cleaned-up branch: its own history shows two "drop redundant" commits removing test additions that duplicated coverage already added via a dedicated new `tests/test_animation_daily_release_ledger.py` file. `final` was an earlier, superseded iteration (its PR #5135 was already closed).
- Verified via a worktree: targeted tests (16) and the full suite (2329 passed, 30 skipped, 0 failed) both green on `digest-final`.
- When I went to open a PR for `digest-final`, GitHub reported one already existed (#5136) — `select_role.py`'s `candidate_reviewable_pr_count` had reported 0 on this same run. Logged as a `LEARNING.yaml` lesson: always re-check `list_pull_requests` by head branch before assuming a worker/* branch has no PR.

**Suggested action:** Merged #5136 (squash), deleted the superseded `final` branch via `branch-janitor.yml force_delete_branches` (session credentials 403 on direct ref deletion, as documented), and opened animation-manager/t-022 to close the root cause — make advancing `daily_last_completed` and recording the matching `PITCHES.yaml` build entry a single documented step instead of two cycles that can silently split apart.

## 2026-09-25 | Agent (Claude, scheduled Conductor Agent run) | animation-manager/t-007 | pattern

**Decision:** merged (cairn-balancing, kind_robots PR #3046)

**What was good:**
- Delegated the full build (component + catalog registration + verification + PR + merge)
  to a worktree-isolated background subagent (hard safety rules 11/12), with the pitch's
  full spec and house-style conventions given up front rather than making the subagent
  hunt for context. It implemented `components/screenfx/cairn-balancing.vue` matching the
  pitch exactly (bounded 6-iteration position-based-dynamics stacking, mass-weighted lean
  tracking, genuine topple/scatter with freeze-to-static-sprite, fixed 7-stone pool,
  pointer breeze + rate-limited click-topple, reduced-motion static frame) and ran the
  project's full verification checklist (eslint, prettier, vue-tsc, test:animation-catalog,
  test:animation-component-attempts, test:layout-contract, test:lint-ratchet,
  check_animation_novelty.py --strict) before pushing.
- All 28 kind_robots CI checks passed on the first push — no retry cycle needed.

**What to improve:**
- The subagent's SubagentHandback fired repeatedly with identical content (6+ times) after
  it had already reported completion, because its own internal background Monitor task
  (polling PR CI) kept waking it back up. Cost some wasted turns until explicitly told to
  cancel that Monitor via TaskStop. Worth a note for future dispatch prompts that ask a
  subagent to poll CI itself: tell it up front to stop its own monitor before its final
  handback, not just to hand back when done.
- `set_task_field.py`/`close_task.py`'s `--set` allow-list doesn't include
  `daily_last_checked`/`daily_last_completed`, so those two fields still require a raw
  hand-edit every cycle instead of going through the same safe tooling as `status`/
  `implementation_pr`. Same gap this project's own t-022 (same-cycle release-ledger
  discipline) was meant to make structural — worth extending the allow-list rather than
  hand-editing YAML every time.

**Kaizen task:** conductor/t-196 — extend `set_task_field.py`/`close_task.py`'s `--set`
allow-list to cover `daily_last_checked`/`daily_last_completed`. Filed against conductor's
own roadmap (tooling issue) rather than this project's, since the gap affects every
`daily_commitment` task across all projects, not just t-007. The Monitor-loop issue is a
session/tooling observation, not a recurring pattern in this project's own build quality —
no task filed for it.
