...
## 2026-09-11 | Agent (Claude, scheduled conductor run) | media-watchlist/t-019 | pattern

type: pattern

**Subject:** The reported symptom ("failed to load, error message masked") had two
independent root causes; code reading found a more important one than the task's
own working hypothesis named.

**Detail:**
- The task's note suspected a stale/mis-scoped browser session for the load failure.
  Reading `watchlist-browse.vue` and `watchlist-entry-detail.vue` instead found that
  every `$fetch` call in the feature was missing the `Authorization` header entirely
  — every request looked anonymous to the server regardless of who was signed in.
  Cross-checked against the working header-attachment pattern already used elsewhere
  in this codebase (`stores/artStore.ts`, `components/art/stylist-clients.vue`) to
  confirm this was a gap specific to the Media Watchlist components, not a
  house-wide absence.
- Fixed both root causes: added the missing `Authorization` header to all five
  `media-entries` `$fetch` calls, and fixed the error-masking half by adding
  `setResponseStatus(event, result.statusCode)` after `errorHandler(error)` in all
  five backend routes, matching the convention already used in
  `achievements/index.get.ts` and `brainstorm/generate.post.ts`.
- The error-masking fix surfaced a much larger house-wide pattern (432 call sites
  across `server/api` return `errorHandler(error)` directly; only 8 already call
  `setResponseStatus`). Filed as `kind-robots/t-096` rather than attempted as a
  drive-by, since fixing the utility itself touches ~400 files and needs a real
  audit + codemod.
- Verified: eslint, vue-tsc --noEmit, prettier --check all clean. No existing test
  coverage for this feature and no live DB/site access from this sandbox to
  reproduce the original failure end-to-end — flagged honestly in the PR rather
  than claimed as fully verified.
- One PR, kind_robots#2628, 48/48 checks green, merged same-session.

**Suggested action:** a human spot-check of `/plan/watchlist` post-deploy is the
strongest remaining confirmation. `kind-robots/t-096` (the house-wide errorHandler
fix) is a reasonable pick for a future session with more budget for a large,
mechanical, well-scoped change.
