...
- [ ] The `Project` row's `liveUrl`, `channelKey`, and `tabKey` are set (via `PROJECT_PLACEMENTS` in kind_robots + `PATCH /api/projects/{id}`, mirrored by hand in `project-overrides.yaml`) and match the page's `dashboardKey`/`dashboardTab`.
