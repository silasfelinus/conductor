...
## 2026-08-01 | Reviewer (Claude session) | kind_robots (untracked) | security-flag

type: security-flag

**Subject:** kind_robots production has failed to deploy for 9 consecutive merges. Every failure is the facet-catalog reseed, and ANY Prisma migration triggers it — so a schema change in one domain currently blocks shipping for the whole site.

**Detail:**

- Last successful production deploy is PR #1253 (`3732a63`, 13:15 UTC). Every merge since has built, applied migrations, then died: #1252 (`b50dba6`), #1254, #1255, #1256, #1257, #1258 (`28fc1db`), #1259 (`f79a08a`). The live site is serving #1253 and is healthy — this blocks SHIPPING, not serving. It also means none of today's merged work is visible, including the interface-vision mockups at `/storymaker-mockups`.

- **The trigger is over-broad.** `scripts/lib/facetCatalogSeedPolicy.mjs:49` ends `isFacetCatalogSourcePath()` with `return normalized.startsWith('prisma/migrations/')`, and `prisma/schema.prisma` is also in `FACET_CATALOG_SOURCE_FILES`. So *any* migration forces a full production catalog reseed. My two interface-vision migrations (`20260801220000_entity_card_hero_icon_art`, `20260801233000_entity_icon_path`) add art columns to Bot/Character/Reward/Scenario and have nothing to do with facets, but they are named verbatim in every failure message as the reason the reseed ran. I did not cause the reseed bug; I did widen its blast radius, and any future schema change will do the same until this is narrowed.

- **The reseed itself has failed two different ways.** Before #1259 it was `P2002` unique-constraint races inside `seedFacetCatalog.ts` (`runWithConcurrency` + `Promise.all`). #1259 correctly serialized it behind a MySQL named lock, and the lock works — but the failure moved: the lock-holding `mariadb` connection is killed mid-run (`ER_CMD_CONNECTION_CLOSED`, `errno 45013`, `sqlState 08S01`). The keepalive ping at `run_facet_catalog_maintenance.ts:189` fails ("Lost the database session holding the catalog lock"), `releaseLock` at :154 then fails, and the child exits 1. Suspected cause: ProxySQL closing a long-lived connection that the maintenance run outlives (this run spanned ~14:15→14:59).

- **Migrations are unaffected and applied.** `prisma migrate deploy` runs early in `vercel-build.mjs` and succeeded in every failed build. Confirmed in #1258's log at 14:15:32: "Applying migration `20260801233000_entity_icon_path` … All migrations have been successfully applied." The production database has the new columns; only the application code is stale. Additive columns unused by older code is a safe state — do NOT attempt to roll the migrations back.

- **Nobody is currently working on it.** There are zero open PRs on kind_robots as of this writing; the session that authored #1259 finished and stopped.

**Suggested action:** FOR SILAS — three options, and this is a judgement call I deliberately did not make unilaterally on a production build pipeline while you were asleep.
1. *Fastest, narrowest:* make the lock-holder resilient — reconnect-and-reacquire on `ER_CMD_CONNECTION_CLOSED` instead of aborting, or shorten the ping interval below ProxySQL's timeout. Fixes the immediate failure without changing when seeding runs.
2. *Addresses the real design issue:* narrow `isFacetCatalogSourcePath` so `prisma/migrations/**` only counts when a migration actually touches facet tables. A schema change in an unrelated domain should not be able to block every deploy. Recommended as the durable fix, alongside (1).
3. *Decouples concerns:* stop treating catalog maintenance as fatal to the build. Arguably correct — a data-maintenance task should not gate shipping code — but it trades a hard failure for silent catalog drift, so it is your call, not an agent's.

Whoever picks this up: `decideFacetCatalogSeed()` is a pure function with an existing checker (`utils/scripts/verifyFacetCatalogSeedPolicy.mjs`), so option 2 is testable without a database. Option 1 is not testable from a sandbox — direct MySQL on :5544 is not reachable there, only the HTTPS API is.
