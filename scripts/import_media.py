...
SOURCE = ROOT / "projects" / "media-watchlist" / "media_list.md"
DATA_DIR = ROOT / "projects" / "media-watchlist" / "data"
...
DATE_TOKEN = re.compile(r"\b(\d{1,2})\s*//?\s*(x|X|\?|\d{1,2})\b(?!\d)")
...
def build_report(entries, narrative, skipped, flagged, section_counts):
    ...
    lines.append("")
    lines.append("Generated by `scripts/import_media.py` from `media_list.md`. Regenerate any time; do not hand-edit.")
    lines.append("")
    lines.append("## Totals")
    lines.append("")
...
def main():
    ...
    total_lines = sum(1 for l in SOURCE.read_text(encoding="utf-8").splitlines() if l.strip())
