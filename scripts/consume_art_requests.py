...
def project_art_sync_payload(entry, art_image_id):
    ...
    project-art requests are still recoverable from `{slug}-{variant}.webp`, so
    the backlog can self-heal without rewriting every historical queue entry.
    """
    ...
    variant = str(entry.get("variant") or (match.group("variant") if match else "")).lower()
    project_field = (
        explicit_field
