...
repo prefix are ambiguous across repos and are skipped, not guessed. Paused,
retired, and finished projects are excluded by default according to
project-overrides.yaml; pass --include-inactive for an intentional archive
sweep.
...
def scan(
    ...
    """Find claimed/review tasks with a resolvable cross-repo PR reference."""
...
def check(
    ...
    `unresolved` holds candidates whose PR lookup failed. These were not
    verified either way and must not be reported as clean.
    """
    findings = []
    unresolved = []
...
def render(
    ...
    if unresolved:
        ...
        for candidate in unresolved:
            lines.append(
                f"    {candidate['project']}/{candidate['task_id']} -> "
                f"{candidate['repo']}#{candidate['pr_number']}"
            )
        lines.append("")
    if findings:
        lines.append(
            f"Found {len(findings)} task(s) at claimed/review whose referenced PR already merged:\n"
        )
        ...
        lines.append(
            f"No drift found — all {total} active-project claimed/review PR reference(s) "
            "verified still open."
        )
    elif len(unresolved) < total:
        lines.append(
            f"No drift found among the {total - len(unresolved)} candidate(s) successfully verified."
        )
    return "\n".join(lines)
...
def main() -> None:
    ...
    candidates = scan(include_inactive=args.include_inactive)
    ...
    # verify. Distinguishing 2 prevents an unverified run from reading as clean.
    if findings:
        sys.exit(1)
    if unresolved:
        sys.exit(2)
