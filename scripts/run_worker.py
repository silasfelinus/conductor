...
Step 3: Claim it with scripts/worker_task_status.py
...
Step 7: Update task status with scripts/worker_task_status.py
...
STATUS_SCRIPT = Path("scripts/worker_task_status.py")
...
def _run_worker_task_status(item: dict, command: str, *extra: str, dry_run: bool = False) -> None:
    if item["is_todo"] or not item["roadmap_path"]:
        return
    task = item["task"]
    args = [sys.executable, str(STATUS_SCRIPT), command, item["project"], task["id"], *extra]
    if dry_run:
        print("  [dry-run] " + " ".join(args))
        return
    result = subprocess.run(args, text=True)
    if result.returncode != 0:
        raise RuntimeError(f"worker_task_status.py failed: {' '.join(args)}")


def claim_task(item: dict, dry_run: bool) -> None:
    if item["is_todo"] or not item["roadmap_path"]:
        return
    _run_worker_task_status(item, "claim", dry_run=dry_run)
    item["task"]["status"] = "claimed"
    item["task"]["owner"] = "worker"
    item["task"]["updated"] = "now"
    commit_msg = f"claim: {item['project']}/{item['task']['id']}"
    if dry_run:
...
def set_task_status(item: dict, status: str, *, dry_run: bool = False) -> None:
    if item["is_todo"] or not item["roadmap_path"]:
        return
    _run_worker_task_status(item, status, dry_run=dry_run)
...
def auto_merge(pr_number: int, branch: str, item: dict, openai_key: str, token: str | None) -> bool:
...
def main() -> None:
    ...
    if decision == "escalate":
        set_task_status(item, "needs-human", dry_run=args.dry_run)
    ...
    set_task_status(item, status_after, dry_run=args.dry_run)
