...
Generated: 2026-07-28T17:20:55Z
...
- 2026-07-28 `coloring-book/t-035` — Extended t-032's recovery-path job-id fix to the other branch of the same loop: a fresh submission whose ArtJob completes and renders but then fails validate_candidate() (missing ANTHROPIC_API_KEY) had no job reference recorded, so referenced_job_id() could never recover it -- only a genuine duplicate resubmission was possible. record_semantic_gate_error() now takes an optional job_id and stamps one on only when the message does not already carry a "job N" reference, so recovery-path messages are never double-stamped and outright enqueue() failures (no ArtJob ever created) stay correctly unstamped. Mirrored t-032's test shape (unit tests + one main()-level integration test) for fast review. No ANTHROPIC_API_KEY in this sandbox to exercise the recovery path live; verified via regression tests only, same constraint every recent t-022/t-032/ t-035 cycle has hit.
...
---
_Auto-generated by `scripts/build_learning_summary.py` at 2026-07-28T17:20:55Z_
